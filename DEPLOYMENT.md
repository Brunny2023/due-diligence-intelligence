...
Fly secrets must be configured server-side with the exact variables expected by the current code: `PINECONE_API_KEY`, `PINECONE_INDEX` or `PINECONE_HOST`, `PINECONE_NAMESPACE`, `LLM_API_KEY`, and `LLM_MODEL`. The current repository implementation uses an OpenAI-compatible client for embeddings and reasoning; it does not currently implement an OpenRouter-specific gateway or Claude Opus 5 model path.
